...
\noindent
A neural network with no hidden layer using the Negative Log-Likelihood loss is equivalent to the softmax loss of the multiclass logit.
...
\item CE: $\displaystyle -\sum_{n=1}^N \sum_{k=1}^K \Big[ \hat{y}_n^{(k)}[y_n = k]  + \sigma\big(-\hat{y}_n^{(k)}\big) \Big]^2 + W$\

\item NLL: $\displaystyle \sum_{n=1}^N \sum_{k=1}^K \Big[ -\log\Big( \text{softmax}(\hat{y}_n^{(k)} [y_n = k]) \Big)  \Big] + W$\\
...
\begin{tabular}{|c|c|c|c|}\hline
 ...
 \verb{params.Nh = [], 5% contaminated{  & 28.1& 33.35& \\ \hline
  \verb{params.Nh = [], 10% contaminated{  & & 37.4&28.75 \\ \hline
 \verb{params.Nh = [], 25% contaminated{  & 30.45& 27.55
& \\ \hline 
...
\noindent
The robust measures, did not seem to help, but I did not sufficiently tune the parameters with respect to these alternative mean squared error formulations - whether it be the "delta" parameter for the Huber loss or the degrees of freedom $\nu$ parameter for the student t log-likelihood. \\

\noindent
While more investigation needs to be done with regards to this experiment, it is evident that the MSE losses do not perform as well as the Cross-Entropy and Negative Log-Likelihood formulations.
